...
Blockchain.prototype.isClientInstalled = function (callback) {
  ...
  this.runCommand(versionCmd, {}, (err, stdout, stderr) => {
    ...
    const supported = this.client.isSupportedVersion(this.client.parseVersion(stdout));
    if (supported === undefined) {
      this.logger.error((__('WARNING! Ethereum client version could not be determined or compared with version range' + ' ' + this.client.versSupported + ', for best results please use a supported version')).yellow);
    } else if (!supported) {
